...
function filterMovies() {
    ...
    const minRate = rateInput.value ? parseFloat(rateInput.value) : null;

    const filteredMovies = movies.filter(movie => {
        if (userId && movie.userId !== userId) {
            return false;
        }
        if (fromDate && new Date(movie.watched) < fromDate) {
            return false;
        }
        if (toDate && new Date(movie.watched) > toDate) {
            return false;
        }
        if (minRate && movie.rate < minRate) {
            return false;
        }
        return true;
    });
...
function displayMovies(movies) {
    ...
    if (movies.length === 0) {
        ...
        moviesList.appendChild(noMoviesMessage);
    } else {
        movies.forEach(movie => {
            ...
            releaseDate.textContent = "Release Date: " + movie.watched;
